...
public class ActivityService {
    ...
    public Activity create(Long projectId, Map<String, Object> payload) {
        ...
        if (payload.get("plannedStartDate") != null) {
            activity.setPlannedStartDate(LocalDate.parse((String) payload.get("plannedStartDate")));
        }
        if (payload.get("plannedEndDate") != null) {
            activity.setPlannedEndDate(LocalDate.parse((String) payload.get("plannedEndDate")));
        }

        Object estimatedHours = payload.get("estimatedHours");
        if (estimatedHours != null) {
            activity.setEstimatedHours(new BigDecimal(estimatedHours.toString()));
        }

        Object isMilestone = payload.get("isMilestone");
        if (isMilestone != null) {
            activity.setMilestone((Boolean) isMilestone);
        }

        Object sequence = payload.get("sequence");
        if (sequence != null) {
            activity.setSequence(((Number) sequence).intValue());
        }
    ...
    public Activity update(Long id, Map<String, Object> payload) {
        ...
        if (payload.get("estimatedHours") != null) {
            activity.setEstimatedHours(new BigDecimal(payload.get("estimatedHours").toString()));
        }
        if (payload.get("isMilestone") != null) {
            activity.setMilestone((Boolean) payload.get("isMilestone"));
        }
        if (payload.get("sequence") != null) {
            activity.setSequence(((Number) payload.get("sequence")).intValue());
        }
    ...
    public ActivityDependency addDependency(Long predecessorId, Long successorId, String dependencyType) {
        ActivityDependency dep = ActivityDependency.builder()
            ...
            .dependencyType(dependencyType != null ? dependencyType : "FS")
            .lagDays(0)
            .build();
    ...
    public void removeDependency(Long predecessorId, Long successorId) {
        var id = new ActivityDependency.Id(predecessorId, successorId);
