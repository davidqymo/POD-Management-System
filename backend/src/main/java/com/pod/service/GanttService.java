...
public class GanttService {
    ...
    public GanttResponse calculateCriticalPath(Long projectId) {
        ...
        // Build adjacency list + indegree map (Kahn's algorithm)
        ...
        // Forward pass: ES = max(pred EF), EF = ES + duration
        ...
        // Backward pass: LF = min(succ LS) or projectEnd if no successors
        ...
        // Build response with critical path
    ...
    private int getDurationDays(Activity a) {
        if (a.getPlannedStartDate() == null || a.getPlannedEndDate() == null) {
            return a.getEstimatedHours() != null
                ? Math.max(1, a.getEstimatedHours().intValue() / 8)
                : 1;
        }
