...
public class ActivityController {
    ...
    public ResponseEntity<?> update(@PathVariable Long projectId, @PathVariable Long id, @RequestBody Map<String, Object> payload) {
        try {
            return ResponseEntity.ok(activityService.update(id, payload));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> addDependency(@PathVariable Long projectId, @RequestBody Map<String, Object> payload) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body(activityService.addDependency(predecessorId, successorId, dependencyType));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> removeDependency(@PathVariable Long projectId, @RequestBody Map<String, Object> payload) {
        try {
            Long predecessorId = ((Number) payload.get("predecessorId")).longValue();
            Long successorId = ((Number) payload.get("successorId")).longValue();
